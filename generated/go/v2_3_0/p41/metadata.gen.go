...
var FieldDescriptions = map[string]map[string]string{
	...
	"BIStateOfServiceAction": {
		"host_regex": "Host name regex.",
		"service_regex": "Service description regex.",
	...
	"BulkDiscovery": {
		...
		"mode": "The mode of the bulk discovery action. The modes 'new', 'remove' and 'only_host_labels' modes give you granular control to monitor undecided services, add new host labels and remove vanished services respectively. The mode 'fix_all' groups those individual actions into one single mode. The 'tabul...",
		"options": "The discovery options for the bulk discovery. The options if specified take precedence over the mode.",
	},
	...
	"CheckboxMatchHostTags": {
		"state": "To enable or disable this field",
		"value": "A list of tag groups or aux tags with conditions. Both 'tag_group_id' and 'tag_id' can't be used more than once.",
	},
	...
	"CreateClusterHost": {
		...
		"host_name": "The host name of the cluster host.",
	...
	"CreateHost": {
		"attributes": "Attributes to set on the newly created host.",
		"folder": "The path name of the folder. Path delimiters can be either `~`, `/` or `\\`. Please use the one most appropriate for your quoting/escaping needs. A good default choice is `~`.",
		"host_name": "The host name or IP address of the host to be created.",
	},
	"CreateHostComment": {
	...
	"CreatePiggybackDCD": {
		"activate_changes_interval": "A delay in seconds can be configured here so that multiple changes can be activated in one go. This avoids frequent activation of changes in situations with frequent services changes.",
		...
		"exclude_time_ranges": "This avoids automatic changes during these times so that the automatic system doesn't interfere with user activity.",
	...
	"HostConditions": {
		...
		"host_labels": "Legacy style host labels will be converted to our new 'host_label_groups'. This field is deprecated and will be removed in a future version. Use 'host_label_groups' instead.",
		"host_tags": "Host tags.",
	},
	"HostConfig": {
		"domainType": "The domain type of the object.",
	...
	"HostDowntimeAttributes": {
		...
		"recurring": "yes if the downtime is recurring, no if it is not.",
		"site_id": "The site id of the downtime.",
	...
	"HostTag": {
		...
		"ident": "An unique id for the tag. This field is deprecated and will be removed in a future version. Use id instead.",
	...
	"InputHostTagGroup": {
		...
		"ident": "An id for the host tag group. This field is deprecated and will be removed in a future version. Use id instead.",
	...
	"JiraPluginCreate": {
		...
		"monitoring_url": "Configure the base URL for the monitoring web GUI here. Include the site name. Used for link to Checkmk out of Jira",
	...
	"MatchHostTags": {
		...
		"tag_type": "If it's an aux tag id or a group tag tag id.",
	},
	"MetaData": {
	...
	"ServiceConditions": {
		...
		"service_labels": "Legacy style service labels will be converted to our new 'service_label_groups'. This field is deprecated and will be removed in a future version. Use 'service_label_groups' instead.",
		"service_regex": "Service description regex.",
	},
	"ServiceDowntimeAttributes": {
		...
		"mode": "The mode of the downtime, either fixed or flexible.",
		"recurring": "yes if the downtime is recurring, no if it is not.",
		"service_description": "The service description if the downtime corresponds to a service, otherwise this field is not present.",
		"site_id": "The site id of the downtime.",
		"start_time": "The start time of the downtime.",
	},
	"ServiceGroup": {
	...
	"TimeIntervalAttribute": {
		"end": "Ending time interval",
		"start": "Starting time interval",
	},
...
var FieldTypes = map[string]map[string]string{
	...
	"CreatePiggybackDCD": {
		"activate_changes_interval": "integer",
		"connector_type": "",
		"creation_rules": "array",
		"discover_on_creation": "boolean",
		"exclude_time_ranges": "array",
		"interval": "integer",
...
func IsRequiredField(schemaName, fieldName string) bool {
	...
	case "AuxHostTag":
		for _, f := range AuxHostTagRequiredFieldNames {
			if f == fieldName {
				return true
			}
		}
	...
	case "TagGroupTag":
		for _, f := range TagGroupTagRequiredFieldNames {
			if f == fieldName {
				return true
			}
		}
	...
	case "TimeIntervalAttribute":
		for _, f := range TimeIntervalAttributeRequiredFieldNames {
			if f == fieldName {
				return true
			}
		}
	case "TimeIntervalAttribute1":
		for _, f := range TimeIntervalAttribute1RequiredFieldNames {
			if f == fieldName {
				return true
			}
		}
...
func IsDeprecatedField(schemaName, fieldName string) bool {
	switch schemaName {
	case "BulkDiscovery":
		for _, f := range BulkDiscoveryDeprecatedFieldNames {
			if f == fieldName {
				return true
			}
		}
	case "HostConditions":
		for _, f := range HostConditionsDeprecatedFieldNames {
			if f == fieldName {
				return true
			}
		}
	case "HostTag":
		for _, f := range HostTagDeprecatedFieldNames {
			if f == fieldName {
				return true
			}
		}
	...
	case "InputHostTagGroup":
		for _, f := range InputHostTagGroupDeprecatedFieldNames {
			if f == fieldName {
				return true
			}
		}
	...
	case "ServiceConditions":
		for _, f := range ServiceConditionsDeprecatedFieldNames {
			if f == fieldName {
				return true
			}
		}
