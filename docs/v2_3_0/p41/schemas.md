...
- [TimeIntervalAttribute](#timeintervalattribute)
- [TimeIntervalAttribute1](#timeintervalattribute1)
...
|----------|------|----------|-------------|
| `state` | string | Yes | To enable or disable this field |
| `value` | []TagTypeSelector | Yes | A list of tag groups or aux tags with conditions. Both 'tag_group_id' and 'ta... |
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `activate_changes_interval` | integer | No | A delay in seconds can be configured here so that multiple changes can be act... |
...
| `exclude_time_ranges` | []TimeIntervalAttribute | No | This avoids automatic changes during these times so that the automatic system... |
...
| `discover_on_creation` | boolean | No | Automatically runs a service discovery on any new hosts created by this conne... |
| `exclude_time_ranges` | []TimeIntervalAttribute1 | No | This avoids automatic changes during these times so that the automatic system... |
| `interval` | integer | No | The interval in seconds the connection will be executed to check the availabl... |
| `max_cache_age` | integer | No | Seconds to keep hosts when piggyback source only sends piggyback data for oth... |
| `no_deletion_time_after_init` | integer | No | Seconds to prevent host deletion after site startup, e.g. when booting the Ch... |
| `restrict_source_hosts` | []string | No | Only care about hosts that receive piggyback data from hosts which name match... |
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `end` | string | Yes | Ending time interval |
| `start` | string | Yes | Starting time interval |
